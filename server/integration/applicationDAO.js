    ...
    try {
        for (let {competenceName, yearsOfExperience } of competences) {
            ...
            if (compRes.rows.length > 0) {
                ...
                await pool.query("INSERT INTO public.competence_profile (person_id, competence_id, years_of_experience) VALUES ($1, $2, $3)", [userData.person_id, competenceId, yearsOfExperience]);
            }
        }
    
        for (let { fromDate, toDate } of availability) {
            await pool.query("INSERT INTO public.availability (person_id, from_date, to_date) VALUES ($1, $2, $3)", [userData.person_id, fromDate, toDate]);  
            console.log("");
            console.log("person id", userData.person_id);
