...
describe("saveApplication", () => {
  it("should successfully save the application with valid data", async () => {
    const userData = { person_id: 1 };
    const competences = [{ competenceName: "Existing Competence", yearsOfExperience: 5 }];
    const availability = [{ fromDate: "2025-01-01", toDate: "2025-02-01" }];
    saveApplication.mockResolvedValue(true);

    const result = await saveApplication(userData, competences, availability);
    expect(result).toBe(true);
  });
});

describe("setStatus", () => {
  it("should successfully update the status with valid data", async () => {
    const person_id = 1;
    const status = 'Approved';
    setStatus.mockResolvedValue({ success: true, message: "Status updated successfully" });

    const result = await setStatus(status, person_id);
    expect(result.success).toBe(true);
    expect(result.message).toBe("Status updated successfully");
  });
});
...
describe("getCompetences", () => {
  it("should return a list of competences", async () => {
    getCompetences.mockResolvedValue(["Competence 1", "Competence 2"]);

    const competences = await getCompetences();
    expect(competences).toEqual(["Competence 1", "Competence 2"]);
  });
});

describe("getAllApplications", () => {
  it("should return a list of all applications", async () => {
    getAllApplications.mockResolvedValue(["Application 1", "Application 2"]);

    const applications = await getAllApplications();
    expect(applications).toEqual(["Application 1", "Application 2"]);
  });
});
